...
All example data is created from several included data files (e.g. `stfFltSum`,`stfFltStkSum`, `stfMtStkSum`), with function-specific preparation shown in the examples of respective plotting functions. For brevity in this tutorial, only the final input objects are shown.
...
Two main objects are used by the `plot_effortFltStk` function: a `data` object and a `refTable` object containing information on plotting colors for added consistency across sevel subsequent plots. 
