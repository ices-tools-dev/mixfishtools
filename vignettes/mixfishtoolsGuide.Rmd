...
Note that the use of `tryNumericSelector = TRUE` may help when selector variable is years, as it will convert these back to numeric values.
...
### Plot absolute values of catch composition for each fleet over time
...
```{r plot_catchComp_output4, out.width="70%", out.height="70%"}
...
Two main objects used by the `plot_alluvial` function: a `data` object and the `refTable` object. 
...
The `data` object contains information for making an alluvial-type plot (e.g. catch or landings). This data.frame should contain several variables for grouping (Default: `group_vars = c("fleet", "metier", "stock")`) as well as a variable to plot values (`value`). The typical `refTable` object is also used to contains the corresponding colors (`col`) and plotting order (`order`) for each level of the grouping variable used for fill colors (Default: `fill_var = "stock"`). See 
`?plot_alluvial` for further settings.
...
```{r plot_alluvial_output2, out.width="70%", out.height="70%"}
p <- plot_alluvial(data = data, refTable = refTable, text_size = 2, 
