...
The following example for nep.fu.6 shows a few tests that ensure the quality control of the FLStock. A TV-survey estimate is used to provide the assumed stock size in the advice year. The discards slot required adjustment to reflect dead discards only. Total discards were replaced and, using the mean discard weight, the discard numbers were re-calculated. The resulting comparison showed only minor difference to the reported values of the advise sheet - possibly due to rounding. 
...
# compute total removals catch.n, landings, dead discards, and harvest rate
...
stfDet <- data.frame(
  ...
  landings = c(landings(stkProj[,ac(ac(yrTAC))])),
  discards = c(discards(stkProj[,ac(ac(yrTAC))])),
  fbar = c(fbar(stkProj[,ac(ac(yrTAC))]))
)
